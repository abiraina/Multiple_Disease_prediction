...
if page == "Liver Disease":
    ...
    total_bilirubin = st.number_input("Total Bilirubin (normal range = 0.1 – 1.2 mg/dL)", min_value=0.0, format="%.2f")
    total_proteins = st.number_input("Total Proteins(normal range = 6.0 – 8.3 g/dL)", min_value=0.0, format="%.2f")
    agr_ratio = st.number_input("Albumin and Globulin Ratio(normal range = 1.0 – 2.1)", min_value=0.0, format="%.2f")
    alt = st.number_input("Alamine Aminotransferase (ALT)(normal range = 7 – 56 U/L)", min_value=0.0, format="%.2f")
    ast = st.number_input("Aspartate Aminotransferase (AST)(normal range = 10 – 40 U/L)", min_value=0.0, format="%.2f")

    # Predict button
    if st.button("Predict"):
        input_data = np.array([[total_bilirubin, total_proteins, agr_ratio, alt, ast]])
    ...
    if st.button("Predict"):
        # Prepare input array
        input_data = np.array([[age, bp, bgr, bu, sc, sod]])
    ...
    if st.button("Predict"):
        ...
        input_data = np.array([[mdvp_shimmer, mdvp_shimmer_db, mdvp_apq, shimmer_dda,
        ...
        # Make prediction
        prediction = pk_model.predict(input_scaled)[0]
